...
const {
  EMAIL,
  FACEBOOK_MAIL_ID,
  FACEBOOK_MAIL_ID_SECRET,
  FACEBOOK_MAIL_REFRESH,
} = process.env;
...
exports.sendVerificationEmail = (email, name, url) => {
  auth.setCredentials({
    refresh_token: FACEBOOK_MAIL_REFRESH,
  });
  ...
  const stmp = nodemailer.createTransport({
    ...
    auth: {
      type: "OAuth2",
      user: EMAIL,
      clientId: FACEBOOK_MAIL_ID,
      clientSecret: FACEBOOK_MAIL_ID_SECRET,
      refreshToken: FACEBOOK_MAIL_REFRESH,
      accessToken,
    },
  });
  const mailOptions = {
